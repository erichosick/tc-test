...
export default async (): Promise<JestConfigWithTsJest> => {
  return {
    ...
    // cacheDirectory: "/private/var/folders/pv/.../T/jest_dx",
    ...
    /// names that allow to stub out resources with a single module. In the
    /// workspace, we need to let jest know which directores contain the source
    /// for the imports. For each package scoped with @, you will need to add
    /// it to the moduleNameMapper.
    moduleNameMapper: {
      '^@(ekhtest|sqlpm)/([^/]+)$': '<rootDir>/packages/@$1/$2/src',
    },
    ...
    /// jest, ts-jest/presets/js-with-ts-esm or ts-jest/presets/default-esm
    ...
    /// Enable adding messages to the expect calls.
    /// See https://github.com/mattphillips/jest-expect-message?tab=readme-ov-file#setup
    setupFilesAfterEnv: [
      'jest-expect-message',
      './.config/jest-global.ts',
    ],
